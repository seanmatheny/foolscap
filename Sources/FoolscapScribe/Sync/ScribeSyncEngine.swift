...
public actor ScribeSyncEngine {
    ...
    public func reloadState() { state = ScribeState.load(from: stateURL) }
    ...
    public func syncOnce(notesRoot: URL, languages: [String],
        ...
        var seen = Set<String>()
        walk(listing, parentID: nil, parentPath: "", root: scribeRoot, seen: &seen)
    ...
    private func walk(_ items: [RemoteItem], parentID: String?, parentPath: String, root: URL, seen: inout Set<String>) {
        for (order, item) in items.enumerated() {
            ...
            if item.isFolder {
                ...
                walk(item.items ?? [], parentID: item.id, parentPath: path, root: root, seen: &seen)
            }
        }
    }
    ...
    private func sync(notebook: ScribeItem, title: String, root: URL, languages: [String],
        ...
        // notebook is fetched every pass and rebuilt only when its page images differ.
        let recheck = !shouldRender && !placeholder
            && now().timeIntervalSince1970 - Double(opened.modificationTime) < Self.recheckWindow
        ...
        if shouldRender || recheck {
            ...
            if shouldRender || hash != entry.contentHash {
                let pdf = try PDFBuilder.makePDF(pages: pages)
                try FileIO.write(pdf, to: pdfURL)
                entry.updateTime = Int(now().timeIntervalSince1970)
        ...
        if let hash = entry.contentHash, !placeholder, fm.fileExists(atPath: pdfURL.path),
           entry.transcribedHash != hash || !fm.fileExists(atPath: mdURL.path) {
            ...
            let modified = Date(timeIntervalSince1970: Double(opened.modificationTime))
    ...
    private func prune(keeping seen: Set<String>, root: URL) -> Int {
        ...
        for item in state.items.values where !seen.contains(item.id) {
            if item.isFolder {
                ...
                if let contents = try? fm.contentsOfDirectory(atPath: dir.path), contents.isEmpty { try? fm.removeItem(at: dir) }
            } else {
                for ext in ["pdf", "md"] { try? fm.removeItem(at: root.appendingPathComponent(item.path + "." + ext)) }

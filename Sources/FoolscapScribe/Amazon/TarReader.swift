...
public enum TarReader {
    ...
    /// GNU long names, directories) are skipped along with their data. The
    /// archive is read in place and each member's data is a slice of it, so
    /// the page images are never copied.
    public static func members(in data: Data) throws -> [Member] {
        ...
        return try data.withUnsafeBytes { bytes in
            ...
            while offset + 512 <= bytes.count {
                let header = UnsafeRawBufferPointer(rebasing: bytes[offset..<offset + 512])
                ...
                if typeflag == 0 || typeflag == UInt8(ascii: "0") {
                    out.append(Member(name: fullName, data: data[(data.startIndex + start)..<(data.startIndex + end)]))
                }
    ...
    private static func isValidHeader(_ header: UnsafeRawBufferPointer) -> Bool {
        guard header[257..<262].elementsEqual("ustar".utf8) else { return false }
    ...
    private static func octal(_ header: UnsafeRawBufferPointer, at field: Int, length: Int) -> Int {
        var value = 0
        for b in header[field..<field + length] {
    ...
    private static func string(_ header: UnsafeRawBufferPointer, at field: Int, length: Int) -> String {
        String(decoding: header[field..<field + length].prefix { $0 != 0 }, as: UTF8.self)
    }
}
